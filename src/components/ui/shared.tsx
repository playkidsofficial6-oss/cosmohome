...
export function Preloader({ onDone }: { onDone: () => void }) {
  ...
  return (
    <motion.div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#1C0E08]"
      ...
      <motion.svg viewBox="0 0 123.8 216" className="w-24 md:w-32 h-auto mb-6 z-10">
        <motion.path 
      ...
      <motion.div className="relative flex flex-col items-center z-10"
        initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.3, duration: 0.8, ease: EASE }}>
        <h1 className="text-[#a6906c] text-3xl md:text-4xl uppercase tracking-[0.15em]" style={D}>
          Cosmo Home
        </h1>
        <p className="text-[#7d5745] text-[9px] md:text-[11px] tracking-[0.3em] uppercase mt-2" style={M}>
          Skin Care Centre
        </p>
      </motion.div>

      <motion.p className="relative mt-5 text-sm md:text-base text-[#a6906c] z-10" style={{ ...D, fontStyle: "italic" }}
        initial={{ opacity: 0, textShadow: "0 0 0px rgba(166,144,108,0)" }} 
        animate={{ opacity: 1, textShadow: "0 0 15px rgba(166,144,108,0.5)" }} 
